...
public class AM_StrtLeftSide_Swch_PICntrl extends CommandGroup {
    ...
    public AM_StrtLeftSide_Swch_PICntrl() {
    	...
    	{
        	addSequential(new AC_DriveEncdrByDistPI((float)17.0, (float)1.0, (float)2.0, (float)0.10, (float)0.0, true, true));
        	addSequential(new AC_TurnByGyroPI(true, 90.0, true)); // Turn CW to 90 deg
    	  	addSequential(new AC_TimeDelay(K_CmndCal.KCMD_t_PostMoveDly));
        	addSequential(new AC_ResetEncoders(K_CmndCal.KCMD_t_EncdrRstDly));
        	addSequential(new AC_DriveEncdrByDistPI((float)16.0, (float)1.0, (float)2.0, (float)0.10, (float)90.0, true, true));
    	    addSequential(new AC_TurnByGyroPI(true, 180.0, true)); // Turn CW to 180 deg
    		addSequential(new AC_TimeDelay(K_CmndCal.KCMD_t_PostMoveDly));
        	addSequential(new AC_ResetEncoders(K_CmndCal.KCMD_t_EncdrRstDly));
        	addSequential(new AC_DriveEncdrByDistPI((float)4.0, (float)0.8, (float)1.0, (float)0.20, (float)180.0, true, true));
