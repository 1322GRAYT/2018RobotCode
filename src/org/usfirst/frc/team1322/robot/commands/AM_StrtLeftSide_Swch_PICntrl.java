    ...
    public AM_StrtLeftSide_Swch_PICntrl() {
    	...
    	if((Robot.kAUTON.getScaleDataCaptured() == true) &&
    	   (Robot.kAUTON.getOurSwitchLeftSide() == true)) { // Our Alliance Switch is on Left Side
        	...
        	addSequential(new AC_ResetEncoders(K_CmndCal.KCMD_t_EncdrRstDly));    	
        	addSequential(new AC_DriveEncdrByDistPI((float)2.0, (float)0.6, (float)0.5, (float)0.20, (float)90.0, false, false));
    	    addSequential(new AC_DriveByGyroTime(-0.5, 0.0, 2.5, false));  // Strafe to the Left
       		addSequential(new AC_TimeDelay(K_CmndCal.KCMD_t_PostMoveDly));
        	addSequential(new AC_ResetEncoders(K_CmndCal.KCMD_t_EncdrRstDly));
        	addSequential(new AC_DriveEncdrByDistPI((float)4.0, (float)0.8, (float)1.0, (float)0.20, (float)90.0, true, false));
        	addSequential(new AC_TurnByGyroPI(true, 180.0, false)); // Turn CW to 180 deg
    	}
        	...
        	addSequential(new AC_DriveEncdrByDistPI((float)17.0, (float)0.9, (float)2.0, (float)0.10, (float)0.0, true, true));
        	addSequential(new AC_TurnByGyroPI(true, 90.0, true)); // Turn CW to 90 deg
    	  	addSequential(new AC_TimeDelay(K_CmndCal.KCMD_t_PostMoveDly));
        	addSequential(new AC_ResetEncoders(K_CmndCal.KCMD_t_EncdrRstDly));
        	addSequential(new AC_DriveEncdrByDistPI((float)16.0, (float)0.9, (float)2.0, (float)0.10, (float)90.0, true, true));
    	    addSequential(new AC_TurnByGyroPI(true, 180.0, true)); // Turn CW to 180 deg
    		addSequential(new AC_TimeDelay(K_CmndCal.KCMD_t_PostMoveDly));
        	addSequential(new AC_ResetEncoders(K_CmndCal.KCMD_t_EncdrRstDly));
        	addSequential(new AC_DriveEncdrByDistPI((float)4.0, (float)0.8, (float)0.9, (float)0.20, (float)180.0, true, true));
