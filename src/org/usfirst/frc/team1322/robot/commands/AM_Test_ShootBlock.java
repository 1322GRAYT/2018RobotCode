...
/**
 * Command Group Class: AM_Test_ShootBlock
 * Autonomous Control Group Pattern: Test for Debugging
 * the Claw Wheel Shoot Block Out Command.
 */
public class AM_Test_ShootBlock extends CommandGroup {
    ...
    public AM_Test_ShootBlock() {   	
	    ...
	    addSequential(new BM_LiftHoldPstn(K_LiftCal.KLFT_r_LiftMtrHldPwrMid));
    	addParallel(new AC_DriveEncdrByDistPI((float)5.0, (float)0.7, (float)0.75, (float)0.10, (float)0.0, true));	    
	    addSequential(new BM_LiftHoldPstn(K_LiftCal.KLFT_r_LiftMtrHldPwrMid));
	    addSequential(new BM_ClawTiltDown());
		addParallel(new AC_TimeDelay((float)20.0));
	    addSequential(new BM_LiftHoldPstn(K_LiftCal.KLFT_r_LiftMtrHldPwrMid));
		...
		addParallel(new AC_TimeDelay((float)10.0));
	    addSequential(new BM_LiftHoldPstn(K_LiftCal.KLFT_r_LiftMtrHldPwrHigh));
    	addParallel(new AC_ResetEncoders(K_CmndCal.KCMD_t_EncdrRstDly));    	
	    addSequential(new BM_LiftHoldPstn(K_LiftCal.KLFT_r_LiftMtrHldPwrHigh));	    
    	addParallel(new AC_DriveEncdrByDistPI((float)5.0, (float)0.7, (float)0.75, (float)0.10, (float)0.0, false));	    
	    addSequential(new BM_LiftHoldPstn(K_LiftCal.KLFT_r_LiftMtrHldPwrHigh));		
    	System.out.println("DriveShootBlockTest");
    }    
